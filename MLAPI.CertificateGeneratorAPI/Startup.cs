...
namespace MLAPI.CertificateGeneratorAPI
{
    public class Startup
    {
        ...
        {            
            ...
            {
                if (context.Request.Method != "GET" || (context.Request.Path.HasValue && context.Request.Path.Value != "/"))
                {
                    context.Response.StatusCode = 404;
                    await context.Response.WriteAsync("Fuck off cunt Id=" + context.Connection.Id + " Address=" + context.Connection.RemoteIpAddress);
                }
                else
                {
